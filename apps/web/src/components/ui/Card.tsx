...
const hoverStyles =
  "hover:-translate-y-1 hover:shadow-hover hover:border-imperial-gold/30 transition-all duration-300";
...
export const Card = ({
  ...
  return (
    <motion.div
      ...
      className={["rounded-xl", base, hoverClass, clickClass, className]
        .filter(Boolean)
        .join(" ")}
      onClick={onClick}
    >
      {(header || icon) && (
        <div className="border-b border-border px-8 py-6">
          <div className="flex items-center gap-3">
            {icon && <div className="text-imperial-gold">{icon}</div>}
            {header && (
      ...
      {footer && (
        <div className="border-t border-border px-8 py-4">{footer}</div>
      )}
    </motion.div>
  );
};
