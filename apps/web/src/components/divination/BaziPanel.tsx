...
function calculateDaYun(
  ...
  gender: "male" | "female",
  birthYear: number,
): Array<{ age: string; pillar: string; years: string }> {
  ...
  const isForward =
    (gender === "male" &&
      ["甲", "丙", "戊", "庚", "壬"].includes(bazi.day.tg)) ||
    (gender === "female" &&
      ["乙", "丁", "己", "辛", "癸"].includes(bazi.day.tg));

  // 获取月柱索引作为大运起始
  const monthIndex =
    TIANGAN.indexOf(bazi.month.tg) * 12 + DIZHI.indexOf(bazi.month.dz);
...
export const BaziPanel = () => {
  ...
  const [paipanMode, setPaipanMode] = useState<"time" | "manual" | "instant">(
    "time",
  );
  ...
  const [timezone, setTimezone] = useState<"beijing" | "solar" | "local">(
    "beijing",
  );
  ...
  const handlePaipan = () => {
    ...
    if (paipanMode === "instant") {
      ...
      result = performPaipan(2000, 1, 1, 0, 0, gender, timezone, 子时算明天, {
        year: manualYear,
        month: manualMonth,
        day: manualDay,
        time: manualTime,
    ...
    <div className="space-y-6">
        ...
        {paipanMode === "manual" && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <Input
              ...
              inputSize="lg"
              className={
                manualYear && !isValidPillar(manualYear) ? "border-red-500" : ""
              }
            />
            <Input
              label="月柱"
              ...
              inputSize="lg"
              className={
                manualMonth && !isValidPillar(manualMonth)
                  ? "border-red-500"
                  : ""
              }
            />
            <Input
              ...
              inputSize="lg"
              className={
                manualDay && !isValidPillar(manualDay) ? "border-red-500" : ""
              }
            />
            <Input
              label="时柱"
              ...
              inputSize="lg"
              className={
                manualTime && !isValidPillar(manualTime) ? "border-red-500" : ""
              }
            />
          </div>
        )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div>
              ...
              <select
                value={timezone}
                onChange={(e) =>
                  setTimezone(e.target.value as "beijing" | "solar" | "local")
                }
        ...
        >
            ...
            <div className="grid grid-cols-4 gap-4">
              {["年柱", "月柱", "日柱", "时柱"].map((label, idx) => {
                ...
                return (
                  ...
                  >
                    <div className="text-sm text-[#D4AF37]/70 mb-2">
                      {label}
                    </div>
                    <div
                      className={`text-4xl font-song ${isDay ? "text-[#D4AF37]" : "text-text-primary"}`}
                    >
                      {pillar.tg}
                    </div>
                    <div
                    ...
                    <div
                      className={`text-sm mt-2 ${isDay ? "text-[#D4AF37]" : "text-text-muted"}`}
                    >
                      {pillar.shishen}
                    </div>
                  </div>
            ...
            <div className="mt-6 text-center">
              <span className="text-text-secondary font-kai">
                ...
                <span className="text-[#D4AF37] font-semibold">
                  {baziResult.dayTiangan}
                  {TIANGAN_WUXING[baziResult.dayTiangan]}
                </span>
              </span>
            </div>
          </Card>
          ...
          <Card header="五行分布">
            <div className="space-y-4">
              {Object.entries(baziResult.wuxingCount).map(
                ([element, count]) => (
                  <div key={element}>
                    <div className="flex items-center justify-between mb-2">
                      <span className="font-kai text-text-primary">
                        {element}
                      </span>
              ...
              {SHISHEN_NAMES.map((name) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div
                      className={`text-2xl font-song ${count > 0 ? "text-[#D4AF37]" : "text-text-muted/50"}`}
                    >
                      {count}
                    </div>
                  </div>
            ...
            <div className="grid grid-cols-4 gap-4">
              {[
                ...
                <div
                  ...
                  className="text-center p-3 bg-bg-tertiary rounded-lg"
                >
                  <div className="text-xl font-song text-text-primary mb-2">
                    {dz}
                  </div>
                ...
                <thead>
                  <tr>
                    {baziResult.daYun.map((yun, idx) => (
                      <th
                        key={idx}
                        className="text-center py-2 px-3 text-sm text-[#D4AF37]"
                      >
                        {yun.age}
                      </th>
                    ))}
                  ...
                  <tr>
                    {baziResult.daYun.map((yun, idx) => (
                      <td key={idx} className="text-center py-3 px-3">
                        <div className="text-xl font-song text-text-primary">
                          {yun.pillar}
                        </div>
      ...
      <Card className="fixed bottom-0 left-0 right-0 bg-[#12121A]/95 backdrop-blur-sm border-t border-[#D4AF37]/10 rounded-none">
        <div className="flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <Button onClick={handleInstantPaipan}>即时排盘</Button>
        </div>
      </Card>
