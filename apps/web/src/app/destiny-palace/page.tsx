...
export default function DestinyPalacePage() {
  ...
  const loadData = useCallback(
    async (y?: string, m?: string, d?: string, g?: string) => {
      const yr = y ?? year;
      const mo = m ?? month;
      const dy = d ?? day;
      const gd = g ?? gender;
      ...
      try {
        ...
        const [baziRes, ziweiRes] = await Promise.all([
          fetch(
            `${API}/api/bazi/calculate?year=${yr}&month=${mo}&day=${dy}&gender=${gParam}`,
          ),
          fetch(
            `${API}/api/ziwei/layout?year=${yr}&month=${mo}&day=${dy}&gender=${gLabel}`,
          ),
        ]);
  ...
  const pillars: { label: string; gan: string; zhi: string }[] = [
    {
      label: "年",
      gan: (gz.year || baziData?.yearPillar || "——").charAt(0),
      zhi: (gz.year || baziData?.yearPillar || "——").charAt(1),
    },
    {
      label: "月",
      gan: (gz.month || baziData?.monthPillar || "——").charAt(0),
      zhi: (gz.month || baziData?.monthPillar || "——").charAt(1),
    },
    {
      label: "日",
      gan: (gz.day || baziData?.dayPillar || "——").charAt(0),
      zhi: (gz.day || baziData?.dayPillar || "——").charAt(1),
    },
    {
      label: "时",
      gan: (gz.hour || baziData?.hourPillar || "——").charAt(0),
      zhi: (gz.hour || baziData?.hourPillar || "——").charAt(1),
    },
  ];
              ...
              <div className="flex-1 grid grid-cols-4 gap-3">
                {pillars.map((p, i) => (
                  ...
                  >
                    ...
                    <div className="text-lg text-gray-400 font-serif mt-0.5">
                      {p.zhi}
                    </div>
                  </div>
                ))}
            ...
            <div className="space-y-3 max-w-md">
              {wuxingOrder.map((wx) => (
                <AnimatedBar
                  ...
                  percent={Math.min(
                    100,
                    Math.max(5, wuxingStrengths[wx] || 20),
                  )}
                />
              ))}
            </div>
            ...
            {ziweiData?.palaces ? (
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {ziweiData.palaces.slice(0, 12).map((p: any, i: number) => {
                  ...
                  return (
                    ...
                    >
                      <div className="text-sm text-gold-400 font-serif mb-1.5">
                        {name}
                      </div>
                      <div className="flex flex-wrap gap-1">
                        {(p.mainStars || []).map((s: string, j: number) => (
                          <Badge key={j} variant="gold" size="sm">
                            {s}
                          </Badge>
                        ))}
                        {(p.auxStars || [])
                          ...
                          .map((s: string, j: number) => (
                            <Badge key={j} variant="ghost" size="sm">
                              {s}
                            </Badge>
                          ))}
                      </div>
              ...
              <div className="text-center py-8 text-gray-500">
                {ziweiData?.mainPalace ? (
                  <p>
                    命宫: {ziweiData.mainPalace}, 主星: {ziweiData.mainStar},
                    运势: {ziweiData.luck || ziweiData.score || "—"}
                  </p>
                ) : (
                  "暂无数据"
                )}
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 justify-items-center">
              ...
              <FortuneCircle
                ...
                score={
                  55 +
                  Math.floor(
                    (ziweiData?.luck || ziweiData?.healthScore || 50) * 0.5,
                  )
                }
                color="#4ade80"
              />
