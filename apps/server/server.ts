...
const server = http.createServer(async (req, res) => {
  ...
  try {
        ...
        data: {
          systems: [
            ...
            },
            {
              id: "liuyao",
              name: "六爻纳甲",
              description: "铜钱/数字/时间起卦，本卦变卦六亲六神",
              inputMode: "random|time",
            },
            {
              id: "bazi",
              name: "八字命理",
              description: "四柱干支、五行强弱、大运流年、喜用神",
              inputMode: "birth",
            },
            {
              id: "qimen",
              name: "奇门遁甲",
              description: "九宫飞星、八门八神、格局检测",
              inputMode: "time",
            },
            {
              id: "ziwei",
              name: "紫微斗数",
              description: "十二宫、十四主星、运势预测",
              inputMode: "birth",
            },
            {
              id: "liuren",
              name: "大六壬",
              description: "月将加时、天地盘、四课三传",
              inputMode: "eventTime",
            },
            {
              id: "xiaochengtu",
              name: "小成图",
              description: "归藏法起卦、九宫排列、主客分析",
              inputMode: "random|time",
            },
            {
              id: "huangli",
              name: "老黄历",
              description: "每日宜忌、冲煞吉神、农历查询",
              inputMode: "date",
            },
            {
              id: "huangji",
              name: "皇极经世",
              description: "元会运世、卦命推演、时间周期",
              inputMode: "birth",
            },
            {
              id: "cegui",
              name: "策轨数",
              description: "动植数计算、卦轨推演",
              inputMode: "birth",
            },
          ],
        },
    ...
    if (path === "/api/predict" && req.method === "POST") {
      ...
      const systems: any[] = Array.isArray(systemField)
        ? systemField
        : [systemField];
        ...
        birth:
          year && month && day
            ? {
                ...
                gender: (url.searchParams.get("gender") || "male") as
                  | "male"
                  | "female",
                calendar: (url.searchParams.get("calendar") || "solar") as
                  | "solar"
                  | "lunar",
              }
            : undefined,
    ...
    if (path === "/api/bazi/calculate") {
      ...
      const day = parseInt(
        url.searchParams.get("day") || String(new Date().getDate()),
      );
      const gender = (url.searchParams.get("gender") || "male") as
        | "male"
        | "female";
    ...
    if (path === "/api/liuren/calculate") {
      ...
      const hour = parseInt(
        url.searchParams.get("hour") || String(new Date().getHours()),
      );
...
server.listen(PORT, () => {
  ...
  console.log(
    `  Pipeline:  http://localhost:${PORT}/api/predict?question=今日运势&system=meihua`,
  );
  console.log(`  梅花易数:  http://localhost:${PORT}/api/meihua/divinate`);
  console.log(`  六爻:      http://localhost:${PORT}/api/liuyao/divinate`);
  console.log(
    `  八字:      http://localhost:${PORT}/api/bazi/calculate?year=1990&month=1&day=1&gender=male`,
  );
  console.log(`  奇门:      http://localhost:${PORT}/api/qimen/layout`);
  console.log(
    `  紫微:      http://localhost:${PORT}/api/ziwei/layout?year=1990&month=1&day=1\n`,
  );
});
