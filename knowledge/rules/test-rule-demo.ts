...
console.log(
  `  成功概率: ${(probMappingResult.probabilityScore.successProbability * 100).toFixed(0)}%`,
);
console.log(
  `  置信度: ${(probMappingResult.probabilityScore.confidence * 100).toFixed(0)}%`,
);
console.log();
...
const executionResults =
  engine.executeRulesWithConflictResolution(sampleContext);
...
interpretation.keySignals.slice(0, 5).forEach((sig) => {
  const polarityMark =
    sig.signal.polarity === "positive"
      ? "+"
      : sig.signal.polarity === "negative"
        ? "-"
        : "~";
...
console.log(
  `  因素: ${interpretation.riskAssessment.factors.join(", ") || "无"}`,
);
console.log("\n【机遇评估】");
console.log(`  等级: ${interpretation.opportunityAssessment.level}`);
console.log(
  `  因素: ${interpretation.opportunityAssessment.factors.join(", ") || "无"}`,
);
console.log("\n【行动建议】");
