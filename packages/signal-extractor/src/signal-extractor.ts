...
export class SignalExtractor {
   ...
   * 提取梅花易数信号
   */
  extractMeihua(result: MeihuaResult, config?: ExtractionConfig): ExtractedSignals {
    const signals: Signal[] = [];
    const keyFindings: string[] = [];

    // 体用关系信号
    if (result.tiYong) {
      const tiYongSignal = MEIHUA_TIYONG_SIGNALS[result.tiYong.relation as string];
      if (tiYongSignal) {
        signals.push(this.createSignal(
          'meihua_tiyong', tiYongSignal.description, tiYongSignal.polarity as any, tiYongSignal.strength as any));
    ...
    if (result.dongYaoPositions && result.dongYaoPositions.length > 0) {
      const polarity: SignalPolarity = result.dongYaoPositions.length > 2 ? 'unstable' : 'neutral';
      signals.push(this.createSignal(
        'meihua_dongyao',
   ...
   * 提取六爻信号
   */
  extractLiuYao(result: LiuYaoResult, config?: ExtractionConfig): ExtractedSignals {
    const signals: Signal[] = [];
    const keyFindings: string[] = [];
    ...
    if (dongYaoCount.length > 0) {
      const polarity: SignalPolarity = dongYaoCount.length > 2 ? 'unstable' : 'neutral';
      signals.push(this.createSignal(
        'liuyao_dongyao',
  ...
  extractBaZi(result: BaZiResult, config?: ExtractionConfig): ExtractedSignals {
    ...
    // 日主强弱信号
    const dmStrengthSignal: SignalPolarity = result.dayMasterStrength === 'balanced' ? 'positive' : 'neutral';
    signals.push(this.createSignal(
      'bazi_rizhu',
    ...
    keyFindings.push(`日主${result.dayMasterStrength === 'balanced' ? '中和' : result.dayMasterStrength}`);
    ...
    const isFavorable = result.favorableWuxing.includes(currentYear.tianganWuxing) || 
