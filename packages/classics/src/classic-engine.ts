...
import { ClassicContent, Chapter, Verse, SchoolOfInterpretation, LineAnnotation } from './types';
import { ZHOUYI_CONTENT, ZHOUYI_INTERPRETATION_SCHOOLS } from './zhouyi-content';
...
export class ClassicEngine {
  ...
  getVerse(classicId: string, chapterId: string, verseNumber: number): Verse | undefined {
  ...
  getInterpretationForLine(
    ...
    const annotation = verse?.lineAnnotations?.find(a => a.lineNumber === lineNumber);
    return annotation?.interpretations?.[schoolId];
  }

  getOriginalText(classicId: string, chapterId: string, verseNumber?: number): string | undefined {
    const verse = verseNumber ? this.getVerse(classicId, chapterId, verseNumber) : undefined;
  ...
  getVernacular(classicId: string, chapterId: string, verseNumber?: number): string | undefined {
    const verse = verseNumber ? this.getVerse(classicId, chapterId, verseNumber) : undefined;
