...
export class RuleBasedInterpreter {
  ...
  constructor(options?: {
    ...
    if (options?.maxSignalsToShow)
      this.maxSignalsToShow = options.maxSignalsToShow;
  }
  ...
  generateInterpretation(
    ...
    const appliedRules = ruleExecutionResults.filter(
      (r) => r.executed && r.matched && r.appliedEffects.length > 0,
    );
    const skippedRules = ruleExecutionResults.filter(
      (r) => !r.executed || !r.matched || r.skippedEffects.length > 0,
    );

    const ruleReferences = this.buildRuleReferences(appliedRules).slice(
      0,
      this.maxRulesToShow,
    );
    const skippedRuleReferences = this.buildRuleReferences(skippedRules).slice(
      0,
      this.maxRulesToShow,
    );
    const signalReferences = this.buildSignalReferences(
      signals,
      appliedRules,
    ).slice(0, this.maxSignalsToShow);
    ...
    const opportunityAssessment = this.assessOpportunities(
      signalReferences,
      probabilityScore,
    );
  ...
  private buildRuleReferences(results: RuleExecutionResult[]): RuleReference[] {
    return results.map((result) => ({
      ...
      matchedConditions: result.appliedEffects.map(
        (e) => e.description || e.id,
      ),
      appliedEffects: result.appliedEffects.map(
        (e) => `${e.type}: ${e.action} ${e.value}`,
      ),
    }));
  }
  ...
  private generateAnalysisText(
    ...
    const positiveSignals = signalReferences.filter(
      (s) => s.signal.polarity === "positive",
    ).length;
    const negativeSignals = signalReferences.filter(
      (s) => s.signal.polarity === "negative",
    ).length;
    const successProbability = Math.round(
      probabilityScore.successProbability * 100,
    );
  ...
  private buildDetailedAnalysis(
    ...
    if (probabilityScore.volatility > 0.5) {
      paragraphs.push(
        `局势存在较大变数，不确定性为 ${Math.round((1 - probabilityScore.confidence) * 100)}%。`,
      );
    }
  ...
  private assessRisks(
    ...
    const riskLevel =
      probabilityScore.volatility > 0.7
        ? "critical"
        : negativeSignals.length > 3 ||
            probabilityScore.failureProbability > 0.6
          ? "high"
  ...
  private assessOpportunities(
    ...
    const opportunityLevel =
      probabilityScore.successProbability > 0.8
        ? "excellent"
  ...
  private buildCalculationTrail(
    ...
    const appliedRules = ruleExecutionResults.filter(
      (r) => r.executed && r.matched,
    );
    trail.push(`匹配成功 ${appliedRules.length} 条规则`);

    for (const result of appliedRules.slice(0, 8)) {
      trail.push(
        `→ 规则 "${result.rule.metadata.name}" (${result.rule.metadata.priority}): 应用 ${result.appliedEffects.length} 个效果`,
      );
    }

    trail.push(
      `概率计算: 成功 ${Math.round(probabilityScore.successProbability * 100)}%, 失败 ${Math.round(probabilityScore.failureProbability * 100)}%`,
    );
