import { LunarDate } from "./types";

// 农历信息数组（简化版）
// 每个元素：[
//   year, month1Days, month2Days, ..., month12Days,
//   leapMonth, yearTotalDays, firstDayGanZhiOffset
// ]
const LUNAR_INFO = [
  0x04bd8, 0x04ae0, 0x0a570, 0x054d5, 0x0d260, 0x0d950, 0x16554, 0x056a0, 0x09ad0, 0x055d2,
  0x04ae0, 0x0a5b6, 0x0a4d0, 0x0d250, 0x1d255, 0x0b540, 0x0d6a0, 0x0ada2, 0x095b0, 0x14977,
  0x04970, 0x0a4b0, 0x0b4b5, 0x06a50, 0x06d40, 0x1ab54, 0x02b60, 0x09570, 0x052f2, 0x04970,
  0x06566, 0x0d4a0, 0x0ea50, 0x06e95, 0x05ad0, 0x02b60, 0x186e3, 0x092e0, 0x1c8d7, 0x0c950,
  0x0d4a0, 0x1d8a6, 0x0b550, 0x056a0, 0x1a5b4, 0x025d0, 0x092d0, 0x0d2b2, 0x0a950, 0x0b557,
  0x06ca0, 0x0b550, 0x15355, 0x04da0, 0x0a5d0, 0x14573, 0x052d0, 0x0a9a8, 0x0e950, 0x06aa0,
  0x0aea6, 0x0ab50, 0x04b60, 0x0aae4, 0x0a570, 0x05260, 0x0f263, 0x0d950, 0x05b57, 0x056a0,
  0x096d0, 0x04dd5, 0x04ad0, 0x0a4d0, 0x0d4d4, 0x0d250, 0x0d558, 0x0b540, 0x0b5a0, 0x195a6,
  0x095b0, 0x049b0, 0x0a974, 0x0a4b0, 0x0b27a, 0x06a50, 0x06d40, 0x0af46, 0x0ab60, 0x09570,
  0x04af5, 0x04970, 0x064b0, 0x074a3, 0x0ea50, 0x06b58, 0x055c0, 0x0ab60, 0x096d5, 0x092e0,
  0x0c960, 0x0d954, 0x0d4a0, 0x0da50, 0x07552, 0x056a0, 0x0abb7, 0x025d0, 0x092d0, 0x0cab5,
  0x0a950, 0x0b4a0, 0x0baa4, 0x0ad50, 0x055d9, 0x04ba0, 0x0a5b0, 0x15176, 0x052b0, 0x0a930,
  0x07954, 0x06aa0, 0x0ad50, 0x05b52, 0x04b60, 0x0a6e6, 0x0a4e0, 0x0d260, 0x0ea65, 0x0d530,
  0x05aa0, 0x076a3, 0x096d0, 0x04bd7, 0x04ad0, 0x0a4d0, 0x1d0b6, 0x0d250, 0x0d520, 0x0dd45,
  0x0b5a0, 0x056d0, 0x055b2, 0x049b0, 0x0a577, 0x0a4b0, 0x0aa50, 0x1b255, 0x06d20, 0x0ada0
];

// 基准日期：1900年1月31日（农历庚子年正月初一）
const BASE_YEAR = 1900;
const BASE_DATE = new Date(1900, 0, 31);

// 获取农历年份的天数
function getLunarYearDays(year: number): number {
  let sum = 348;
  const info = LUNAR_INFO[year - BASE_YEAR];
  for (let i = 0x8000; i > 0x8; i >>= 1) {
    sum += (info & i) ? 1 : 0;
  }
  return sum + getLeapMonthDays(year);
}

// 获取农历闰月的天数
function getLeapMonthDays(year: number): number {
  const info = LUNAR_INFO[year - BASE_YEAR];
  return getLeapMonth(year) ? ((info & 0x10000) ? 30 : 29) : 0;
}

// 获取农历某月的天数
function getLunarMonthDays(year: number, month: number, isLeap: boolean = false): number {
  const info = LUNAR_INFO[year - BASE_YEAR];
  if (isLeap) {
    return (info & 0x10000) ? 30 : 29;
  }
  return (info & (0x10000 >> month)) ? 30 : 29;
}

// 获取农历闰月（0=无，1-12=闰哪个月）
function getLeapMonth(year: number): number {
  const info = LUNAR_INFO[year - BASE_YEAR];
  return info & 0xf;
}

// 公历转农历
export function solarToLunar(year: number, month: number, day: number): LunarDate {
  const baseDate = new Date(BASE_YEAR, 0, 31);
  const targetDate = new Date(year, month - 1, day);
  let offset = Math.floor((targetDate.getTime() - baseDate.getTime()) / 86400000);

  let lunarYear = BASE_YEAR;
  let lunarDays = 0;

  while (lunarYear < 2100 && offset > 0) {
    lunarDays = getLunarYearDays(lunarYear);
    if (offset < lunarDays) break;
    offset -= lunarDays;
    lunarYear++;
  }

  let leapMonth = getLeapMonth(lunarYear);
  let lunarMonth = 1;
  let isLeap = false;
  let monthDays = 0;

  for (let i = 1; i <= 12; i++) {
    if (leapMonth && i === leapMonth + 1 && !isLeap) {
      monthDays = getLeapMonthDays(lunarYear);
      isLeap = true;
      i--;
    } else {
      monthDays = getLunarMonthDays(lunarYear, i, false);
    }

    if (offset < monthDays) {
      lunarMonth = i;
      break;
    }

    offset -= monthDays;
    if (isLeap && i === leapMonth) {
      isLeap = false;
    }
  }

  return {
    year: lunarYear,
    month: lunarMonth,
    day: offset + 1,
    isLeap
  };
}

// 获取地支序数（子=1，丑=2...亥=12）
export function getDizhiNumber(dizhi: string): number {
  const dizhiList = ["子", "丑", "寅", "卯", "辰", "巳", "午", "未", "申", "酉", "戌", "亥"];
  return dizhiList.indexOf(dizhi) + 1;
}
