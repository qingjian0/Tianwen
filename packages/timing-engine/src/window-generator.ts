...
export class WindowGenerator {
  ...
  static generateAnalysis(baseDate: Date = new Date()): TimingAnalysis {
    ...
    const recommendations = this.generateRecommendations(currentWindow, upcomingWindows);
  ...
  private static generateUpcomingWindows(baseDate: Date): TimingWindow[] {
    ...
    let currentDate = this.addDays(baseDate, TIME_HORIZON_DAYS[TimeHorizon.SHORT]);
  ...
  private static generateRecommendations(
    ...
    const bestUpcoming = upcomingWindows.reduce((best, w) =>
      w.strength > best.strength ? w : best,
      upcomingWindows[0]
    );
