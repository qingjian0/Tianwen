...
async function runCognitiveDemo() {
  ...
  const context = {
    ...
    rules: [{ id: "rule_wuxing_sheng" }, { id: "rule_tiangan_he" }],
  };
  ...
  console.log(
    `  Inheritance Strength: ${inheritance.inheritanceStrength.toFixed(3)}`,
  );
  console.log(
    `  Inherited Properties: ${Object.keys(inheritance.inheritedProperties).join(", ")}`,
  );
  ...
  if (semanticRels.length > 0) {
    console.log(
      `  First Relation Similarity: ${semanticRels[0].semanticSimilarity.toFixed(3)}`,
    );
  }
  ...
  const fusionResult = await resonanceEngine.fuse(inputs, {
    question: "事业发展如何？",
  });
  ...
  console.log(
    `  Harmonized Signals: ${fusionResult.fusionReasoning.harmonizedSignals.length}`,
  );
  console.log(
    `  Fusion Confidence: ${fusionResult.fusionReasoning.confidence.toFixed(3)}`,
  );
  console.log(`  Final Conclusion: ${fusionResult.finalResult.statement}`);
  console.log(
    `  Probability: ${(fusionResult.finalResult.probability * 100).toFixed(1)}%`,
  );
  console.log(
    `  Supporting Systems: ${fusionResult.finalResult.supportingSystems.join(", ")}`,
  );
  if (fusionResult.finalResult.contradictingSystems.length > 0) {
    console.log(
      `  Contradicting Systems: ${fusionResult.finalResult.contradictingSystems.join(", ")}`,
    );
  }
