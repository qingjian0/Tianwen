...
export class CeGuiEngine {
  ...
  calculate(input: CeGuiInput): CeGuiResult {
    const chrono = ChronoEngine.at(
      new Date(input.year, input.month - 1, input.day, input.hour),
    );

    const yearNum = input.year % 100;
    const dongShu =
      (yearNum + input.month * 3 + input.day * 5 + input.hour * 2) % 72 || 72;
    const zhiShu =
      (yearNum * 3 + input.month + input.day * 7 + input.hour * 4) % 72 || 72;
