...
export class FortuneAnalyzer {
  ...
  private static extractFactors(signals: Signal[]): FortuneFactors {
    ...
    const positiveSignalRatio = analysis.totalSignals > 0
   ...
   * 提取警告
   */
  private static extractWarnings(score: FortuneScore, signals: Signal[]): string[] {
    const warnings: string[] = [];
    const analysis = SignalProcessor.analyze(signals);

    if (score.level === FortuneLevel.DANGER || score.level === FortuneLevel.WARNING) {
   ...
   * 提取建议
   */
  private static extractRecommendations(score: FortuneScore, signals: Signal[]): string[] {
    const recommendations: string[] = [];
    const analysis = SignalProcessor.analyze(signals);

    if (score.level === FortuneLevel.GREAT_FORTUNE || score.level === FortuneLevel.FORTUNE) {
