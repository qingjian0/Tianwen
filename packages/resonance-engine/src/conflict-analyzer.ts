...
export class ConflictAnalyzer {
  ...
  static analyzeConflict(comparison: SignalComparison): {
    ...
    const resolutionSuggestions = this.generateSuggestions(
      comparison,
      severity,
    );
   ...
   * 检查极性冲突
   */
  private static hasPolarityConflict(
    signalsA: Signal[],
    signalsB: Signal[],
  ): boolean {
    const aPositive = signalsA.filter(
      (s) => s.polarity === SignalPolarity.POSITIVE,
    ).length;
    const bPositive = signalsB.filter(
      (s) => s.polarity === SignalPolarity.POSITIVE,
    ).length;
   ...
   * 检查强度冲突
   */
  private static hasStrengthConflict(
    signalsA: Signal[],
    signalsB: Signal[],
  ): boolean {
    const avgStrengthA =
