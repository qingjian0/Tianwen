...
export class QimenEngine {
   ...
   * 计算局数
   */
  private calculateJu(chronoData: ChronoData): { yinYang: '阴' | '阳', ju: number } {
  ...
  private buildNinePalaces(
    ...
    const hourIdx = ['子', '丑', '寅', '卯', '辰', '巳', '午', '未', '申', '酉', '戌', '亥'].indexOf(hourDizhi);
  ...
  private detectPatterns(
    ...
    if (zhiFuPalace && ['天辅', '天心', '天禽'].includes(zhiFuPalace.eightStar)) {
      patterns.push('值符得地');
    }

    const zhiShiPalace = palaces[zhiShi.palace];
    if (zhiShiPalace && ['开门', '休门', '生门'].includes(zhiShiPalace.eightDoor)) {
      patterns.push('值使得地');
    }
   ...
   * 检查宫位冲
   */
  private checkPalaceRushed(xunKong: [Dizhi, Dizhi], position: number): boolean {
   ...
   * 检查宫位合
   */
  private checkPalaceCombined(xunKong: [Dizhi, Dizhi], position: number): boolean {
