...
export class XiaoChengTuEngine {
  ...
  calculate(input: XiaoChengTuInput): XiaoChengTuResult {
    ...
    if (input.numbers && input.numbers.length >= 3) {
      ...
      const n3 =
        (chrono.lunar.year + chrono.lunar.month + chrono.lunar.day) % 6 || 6;
      numbers = [n1, n2, n3];
    } else {
      const now = new Date();
      const ms = now.getMilliseconds();
      numbers = [ms % 8 || 8, (ms * 3 + 7) % 8 || 8, (ms * 7 + 3) % 6 || 6];
    }
    ...
    for (let i = 1; i <= 9; i++) {
      ...
      palaces.push({
        ...
        relationship:
          BAGUA_NAMES[guaNum] === hostGua
            ? "体卦"
            : BAGUA_NAMES[guaNum] === guestGua
              ? "用卦"
              : "辅卦",
  ...
  analyze(result: XiaoChengTuResult): {
    ...
    return {
      ...
      keyPalaces: result.palaces
        .filter((p) => p.isYongShen || p.isHost)
        .map((p) => p.analysis),
