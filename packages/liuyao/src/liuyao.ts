import { ChronoEngine, Tiangan, Dizhi, Wuxing, Bagua, ChronoData } from '@tianwen/chrono-engine';
import {
  NAGUA_MAP,
  LIUQIN_RELATION,
  LIUSHEN,
...
const GUA_NAMES: Record<string, string> = {
  '乾乾': '乾为天', '兑兑': '兑为泽', '离离': '离为火', '震震': '震为雷',
  '巽巽': '巽为风', '坎坎': '坎为水', '艮艮': '艮为山', '坤坤': '坤为地',
  '乾兑': '天泽履', '乾离': '天火同人', '乾震': '天雷无妄', '乾巽': '天风姤',
  '乾坎': '天水讼', '乾艮': '天山遁', '乾坤': '天地否', '兑乾': '泽天夬',
  '兑离': '泽火革', '兑震': '泽雷随', '兑巽': '泽风大过', '兑坎': '泽水困',
  '兑艮': '泽山咸', '兑坤': '泽地萃', '离乾': '火天大有', '离兑': '火泽睽',
  '离震': '火雷噬嗑', '离巽': '火风鼎', '离坎': '火水未济', '离艮': '火山旅',
  '离坤': '火地晋', '震乾': '雷天大壮', '震兑': '雷泽归妹', '震离': '雷火丰',
  '震巽': '雷风恒', '震坎': '雷水解', '震艮': '雷山小过', '震坤': '雷地豫',
  '巽乾': '风天小畜', '巽兑': '风泽中孚', '巽离': '风火家人', '巽震': '风雷益',
  '巽坎': '风水涣', '巽艮': '风山渐', '巽坤': '风地观', '坎乾': '水天需',
  '坎兑': '水泽节', '坎离': '水火既济', '坎震': '水雷屯', '坎巽': '水风井',
  '坎艮': '水山蹇', '坎坤': '水地比', '艮乾': '山天大畜', '艮兑': '山泽损',
  '艮离': '山火贲', '艮震': '山雷颐', '艮巽': '山风蛊', '艮坎': '山水蒙',
  '艮坤': '山地剥', '坤乾': '地天泰', '坤兑': '地泽临', '坤离': '地火明夷',
  '坤震': '地雷复', '坤巽': '地风升', '坤坎': '地水师', '坤艮': '地山谦'
};
...
export class LiuYaoEngine {
   ...
   * 数字起卦
   */
  divinateByNumber(num1: number, num2: number, num3?: number, date?: Date): LiuYaoResult {
    const chronoData = date ? ChronoEngine.at(date) : ChronoEngine.now();
  ...
  divinateByTime(date?: Date): LiuYaoResult {
    ...
    const yaos: YaoState[] = [];
    const seed = (lunar.year + lunar.month + lunar.day + this.getHourIndex(chronoData.shichen.dizhi));
  ...
  private buildResult(
    ...
    // 1. 确定本卦
    const benGuaYaosBinary = yaos.map(y => (y === 'oldYang' || y === 'youngYang') ? '1' : '0').join('');
    ...
    const changingPositions = benGuaYaos.filter(y => y.isChanging).map(y => y.position);
    if (changingPositions.length > 0) {
      const bianBinary = benGuaYaosBinary.split('').map((c, i) => {
        return changingPositions.includes(i + 1) ? (c === '1' ? '0' : '1') : c;
      }).join('');
      ...
      const bianGuaName = GUA_NAMES[bianShangGua + bianXiaGua] || `${bianShangGua}${bianXiaGua}`;

      const bianShiyaoPos = this.getShiyaoPosition(bianShangGua, bianXiaGua);
      const bianYingyaoPos = ((bianShiyaoPos + 2) % 6) || 6;
      const bianShiyaoWuxing = this.getShiyaoWuxing(bianShangGua, bianXiaGua, bianShiyaoPos);

      const bianGuaYaos: LiuYaoYao[] = [];
      for (let i = 0; i < 6; i++) {
        const pos = i + 1;
        const yaoState = changingPositions.includes(pos)
          ? (yaos[i] === 'oldYang' ? 'youngYin' : 'youngYang')
          : yaos[i];
   ...
   * 获取纳甲
   */
  private getNagua(shangGua: Bagua, xiaGua: Bagua, position: number): { tiangan: Tiangan; dizhi: Dizhi } {
   ...
   * 获取世爻五行
   */
  private getShiyaoWuxing(shangGua: Bagua, xiaGua: Bagua, position: number): Wuxing {
