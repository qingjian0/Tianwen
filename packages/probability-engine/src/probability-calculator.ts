...
export class ProbabilityCalculator {
  ...
  static calculateFromSignals(signals: Signal[]): ProbabilityScore {
    ...
    const successProbability = Math.max(0, Math.min(1, (1 + analysis.netPolarity) / 2));
  ...
  private static determineLevel(probability: number): ProbabilityLevel {
    if (probability <= PROBABILITY_THRESHOLDS.VERY_LOW) return ProbabilityLevel.VERY_LOW;
    if (probability <= PROBABILITY_THRESHOLDS.LOW) return ProbabilityLevel.LOW;
    if (probability <= PROBABILITY_THRESHOLDS.MEDIUM) return ProbabilityLevel.MEDIUM;
    if (probability <= PROBABILITY_THRESHOLDS.HIGH) return ProbabilityLevel.HIGH;
  ...
  private static calculateVolatility(signals: Signal[]): number {
    ...
    const polarities = signals.map((s) => {
      switch (s.polarity) {
        case SignalPolarity.POSITIVE: return 1;
        case SignalPolarity.NEGATIVE: return -1;
        case SignalPolarity.UNSTABLE: return 0.5;
        default: return 0;
      }
    });

    const mean = polarities.reduce((a, b) => a + b, 0) / polarities.length;
    const variance = polarities.reduce((sum, p) => sum + Math.pow(p - mean, 2), 0) / polarities.length;
   ...
   * 合并多个概率分数
   */
  static mergeScores(scores: ProbabilityScore[], weights?: number[]): ProbabilityScore {
