...
import { JIANCHU, YI_JI_TEMPLATES, JISHEN_LIST, XIONGSHEN_LIST } from './constants';
...
export class HuangLiEngine {
  ...
  private getApproxJieqi(date: Date): string | null {
    ...
    const jieqiMap: [number, number, string][] = [
      [0, 5, '小寒'], [0, 20, '大寒'], [1, 4, '立春'], [1, 19, '雨水'],
      [2, 5, '惊蛰'], [2, 20, '春分'], [3, 5, '清明'], [3, 20, '谷雨'],
      [4, 5, '立夏'], [4, 21, '小满'], [5, 5, '芒种'], [5, 21, '夏至'],
      [6, 7, '小暑'], [6, 22, '大暑'], [7, 7, '立秋'], [7, 23, '处暑'],
      [8, 7, '白露'], [8, 23, '秋分'], [9, 8, '寒露'], [9, 23, '霜降'],
      [10, 7, '立冬'], [10, 22, '小雪'], [11, 7, '大雪'], [11, 22, '冬至']
    ];
  ...
  private getPengzu(day: number): string[] {
    const pengzuMap: Record<number, string[]> = {
      1: ['甲不开仓', '子不问卜'], 2: ['乙不栽植', '丑不冠带'],
      3: ['丙不修灶', '寅不祭祀'], 4: ['丁不剃头', '卯不穿井'],
      5: ['戊不受田', '辰不哭泣'], 6: ['己不破券', '巳不远行'],
      7: ['庚不经络', '午不苫盖'], 8: ['辛不合酱', '未不服药'],
      9: ['壬不决水', '申不安床'], 10: ['癸不词讼', '酉不会客'],
      11: ['子不问卜', '戌不吃犬'], 12: ['丑不冠带', '亥不嫁娶'],
      13: ['寅不祭祀', '甲不开仓'], 14: ['卯不穿井', '乙不栽植'],
      15: ['辰不哭泣', '丙不修灶'], 16: ['巳不远行', '丁不剃头'],
      17: ['午不苫盖', '戊不受田'], 18: ['未不服药', '己不破券'],
      19: ['申不安床', '庚不经络'], 20: ['酉不会客', '辛不合酱'],
      21: ['戌不吃犬', '壬不决水'], 22: ['亥不嫁娶', '癸不词讼'],
      23: ['甲不开仓', '卯不穿井'], 24: ['乙不栽植', '辰不哭泣'],
      25: ['丙不修灶', '巳不远行'], 26: ['丁不剃头', '午不苫盖'],
      27: ['戊不受田', '未不服药'], 28: ['己不破券', '申不安床'],
      29: ['庚不经络', '酉不会客'], 30: ['辛不合酱', '戌不吃犬'],
      31: ['壬不决水', '亥不嫁娶']
    };
