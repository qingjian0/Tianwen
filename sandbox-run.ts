...
class CognitiveEngine {
  ...
  private analyzeRelations(
    signals: any[],
    trace: { nodes: Map<string, any>; edges: any[] },
  ) {
...
class RuleKnowledgeGraph {
  ...
  addRelation(
    fromRule: string,
    toRule: string,
    type: (typeof this.relationTypes)[number],
    weight = 0.5,
  ) {
  ...
  getStats() {
    return {
      ...
      avgConfidence:
        Array.from(this.nodes.values()).reduce((s, n) => s + n.confidence, 0) /
          this.nodes.size || 0,
    };
  }
}
...
class MultiSystemResonanceEngine {
  ...
  fuse(systems: Array<{ name: string; signals: any[]; confidence: number }>) {
    const totalConfidence = systems.reduce(
      (sum, s) =>
        sum +
        s.confidence *
          (this.systemWeights[s.name as keyof typeof this.systemWeights] ||
            0.5),
      0,
    );
    const totalWeight = systems.reduce(
      (sum, s) =>
        sum +
        (this.systemWeights[s.name as keyof typeof this.systemWeights] || 0.5),
      0,
    );
...
async function runSandbox() {
  ...
  const baziResult = bazi.calculate({
    year: 1990,
    month: 1,
    day: 1,
    gender: "男",
  ...
  const ziweiResult = ziwei.layout({
    year: 1990,
    month: 1,
    day: 1,
    gender: "男",
  ...
  const fusionResult = resonanceEngine.fuse([
    {
      name: "meihua",
      signals: [meihuaResult],
      confidence: meihuaResult.confidence,
    },
    { name: "bazi", signals: [baziResult], confidence: baziResult.confidence },
    {
      name: "qimen",
      signals: [qimenResult],
      confidence: qimenResult.confidence,
    },
    {
      name: "ziwei",
      signals: [ziweiResult],
      confidence: ziweiResult.confidence,
    },
  ]);

  console.log(
    "  融合置信度:",
    (fusionResult.finalConfidence * 100).toFixed(1) + "%",
  );
  ...
  console.log(
    "  推理置信度:",
    (cogResult.finalConfidence * 100).toFixed(1) + "%",
  );
  ...
  console.log(
    "  结果预测:",
    finalScore > 70 ? "✅ 积极" : finalScore > 50 ? "⚪ 中性" : "⚠️ 需谨慎",
  );
